...
enum {
	...
	SRC_BUFFER_USE_NULL     = 0x7FFFFFFF
};
...
typedef struct VP9Superframe {
	...
	PhysicalAddress frames[VP9_MAX_SUBFRAMES];   /* A superframe has multiple frames up to 8 frames. */
	Uint32 currentIndex;
	Uint32 doneIndex;
} VP9Superframe;
...
typedef struct {
	...
	int             previous_frameDisplayFlag;
	Uint32          frameNumCount;
	Uint32          skipframeUsed;
#endif
#ifdef TEST_CQ2_DEC_SKIPFRAME_CQCOUNT
	Uint32          dispidx_backup;
#endif
...
//	vpu_buffer_t    vbFbcCTbl;
	...
	BOOL            userDataEnable;	/* User Data Enable Flag : Refer to H265_USERDATA_FLAG_xxx values in vpuapi.h */
	int             userDataBufSize;
	...
	DecOutputInfo   decOutInfo[MAX_GDI_IDX + 1]; //32 array
...
//	Uint32          prevFrameEndPos;
	...
	int		seqinitdone;
	int		prev_sequencechanged;
	int		decidx_seqchange;
	int		sequencechange_detected;

//	int		decRemainingDispNum;	// TEST_FULLFB_SEQCHANGE_REMAINING
//	int		decRemainingDispIndex;	// TEST_FULLFB_SEQCHANGE_REMAINING

	int		framebufSizeCompressed;
	int		framebufSizeLinear;
	int		framebufSizeCompressedLinear;

	int		mvColSize;
	int		fbcYTblSize;
	int		fbcCTblSize;

	int		queueing_fail_flag;
//	int		vp9Superframe;
	PhysicalAddress prev_streamWrPtr;
	int		prev_buffer_full;	// TEST_BUFFERFULL_POLLTIMEOUT
	int		fb_cleared;	// TEST_BUFFERFULL_POLLTIMEOUT
...
typedef struct CodecInst {
	...
	Int32    loggingEnable;
	Uint32   isDecoder;
	unsigned int bufAlignSize;	// Aligned size of buffer (>= VPU_WAVE5_HEVC_MIN_BUF_SIZE_ALIGN 4Kbytes)
	unsigned int bufStartAddrAlign;	// Aligned start address of buffer (>= VPU_WAVE5_HEVC_MIN_BUF_START_ADDR_ALIGN 4Kbytes)
	union {
		 DecInfo decInfo;
	} CodecInfo;
} CodecInst;
...
} h265_alternative_transfer_characteristics_info_t;
